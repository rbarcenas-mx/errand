...
async function main() {
  ...
  await prisma.$executeRaw`DELETE FROM denuncias`;
  await prisma.$executeRaw`DELETE FROM calificaciones`;
  await prisma.$executeRaw`DELETE FROM mensajes`;
  await prisma.$executeRaw`DELETE FROM ofertas`;
  await prisma.$executeRaw`DELETE FROM mandados`;
  await prisma.$executeRaw`DELETE FROM refresh_tokens`;
  await prisma.$executeRaw`DELETE FROM otp_codes`;
